...
class Prestamo:
    def __init__(self,empleado, aamm, valor, numPagos,saldo, estado= True,id=1):
        ...
        self.cuota = round(valor/numPagos,2)
...
class DetalleNomima(CalculoRol): 
    ...
    def getSobretiempo(self,aamm):
      ...
      if self.empleado.id[0]=="O":
        ...
        sobret = archiSobretiempo.buscar2(self.empleado.id,aamm)
        if sobret:
            entSobret = Sobretiempo(sobret[1],sobret[2],int(sobret[3]),int(sobret[4]),sobret[5],sobret[0])
            return round(self.empleado.valorHora()+(entSobret.h50*0.50+entSobret.h100*2))
            #return entSobret.sobretiempo()
    ...
    def getAntiguedad(self,deduccion):
        ...
        if self.empleado.id[0]=="O":
            ...
            calAnt = round(deduccion.getAntiguedad()*(date.today().year - self.empleado.fechaIngreso)/365*self.empleado.sueldo,2) # traer antiguedad de deducciones y realizar calculo  
            #calAnt = round(deduccion.getAntiguedad()*('#FA 
        return calAnt
